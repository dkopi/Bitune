...
from models.think_gemma import PassScale
...
class ThinkPhiForCausalLM(PhiPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        if not hasattr(config, "pass_type"):
            config.pass_type = 0
    ...
    def forward(
        ...
        if self.config.pass_type > 0 and call_depth == 0 and position_ids is None:
            return _pass_fn(
                peft_model,
                input_ids=input_ids,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if model_inputs["position_ids"][0][0] == 0:
